...
*/
long Fix_MDBSubscibeSopQuoteByCustomer( HANDLE_CONN conn, long nSvc,
		void *pFunc, void *pData, const char *pszKHH, const char *pszPwd,
		const char *pszJYS, const char *pszHYDM);
...
*/
//普通A股订阅
FIX_API_EXPORT long Fix_MDBSubscibeByCustomer( HANDLE_CONN conn, long nSvc, void *pFunc, void *pData,const char *pszKHH, const char *pszPwd );
//融资融券订阅
FIX_API_EXPORT long Fix_MDBSubscibeByCustomer_RZRQ( HANDLE_CONN conn, long nSvc, void *pFunc, void *pData,const char *pszKHH, const char *pszPwd );
//个股期权订阅
FIX_API_EXPORT long Fix_MDBSubscibeByCustomer_SOP( HANDLE_CONN conn, long nSvc, void *pFunc, void *pData,const char *pszKHH, const char *pszPwd );
...
FIX_API_EXPORT long Fix_SubscibeQuoteByZQDM(HANDLE_CONN conn, void *pFunc, void *pData, const char *pszJYS, const char *pszZQDM);

//订阅行情中指定指数代码最新指数信息
//pszJYS  订阅交易所范围，不同交易所使用分号分割譬如SH;SZ，空串表示订阅所有交易所
//pszIndexCode 订阅指数范围，代码区间使用逗号分割，譬如订阅000001--000100表达成000001,000100，独立代码使用分号分割，譬如订阅000033和000066表达成000033;000066
FIX_API_EXPORT long Fix_SubscibeIndexByCode(HANDLE_CONN conn, void *pFunc, void *pData, const char *pszJYS, const char *pszIndexCode);
...
//设置应答回调函数。V2.0.3.5新增函数
/*
	1、如果sess为null, 表示设置整个系统的全局应答回调函数。
	2、如果sess不为空, 则针对莫个业务请求的设置应答回调函数。
	回调函数格式如下:	
	bool OnSessRely( HANDLE_CONN hConn, HANDLE_SESSION hSess, int nRecv )
	{
		//nRecv=1 表示通信正常，通过hSess句柄读取应答数据
...
FIX_API_EXPORT BOOL Fix_GetValWithIdByIndex( HANDLE_SESSION sess, int nRow, int nIndex, long &nId, char *pBuffer, int &nSize );
///------------快速交易专用函数结束------------------------------------------------------
#endif
